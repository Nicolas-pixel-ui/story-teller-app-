-- Harden consume_user_credit: reject non-positive or oversized client amounts.
-- Prevents authenticated callers from minting credits via negative p_amount.
...
BEGIN
  ...
  -- Only allow a positive debit within a safe bound (daily quota).
  IF p_amount IS NULL OR p_amount <= 0 OR p_amount > 140 THEN
    RETURN jsonb_build_object(
      'ok', false,
      'code', 'INVALID_AMOUNT',
      'message', 'Credit amount must be between 1 and 140',
      'balance', 0
    );
  END IF;
  v_amount := p_amount;
  ...
  SET
    ...
    monthly_used = monthly_used + v_amount,
    updated_at = now()
  WHERE user_id = v_user_id
